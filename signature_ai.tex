...
\begin{abstract}
    ...
    limiting update frequency. One possible solution for the frequency issue comes from
    ...
    Our focus is not only to develop a performant predictive model but also to learn
    about the importance of geographically-explicit methods of doing so. The initial
    results indicate that classification focusing on predominantly urban environments is
    more challenging than that dealing primarily with non-urban areas. However, the
    accuracy of a subset of the tested models is either on par or in the neighbourhood
    of established land cover classification models. This suggests that satellite
    imagery can, with some limitations, reflect form and function of urban environment
    in a greater detail than is usually present in available remote sensing products.
    This paper presents initial exploratory work and empirical experiments focusing on
    these tasks and discusses the opportunities and challenges in using remote sensing
    to reliably detect concepts like spatial signatures using openly available satellite
    imagery.
\end{abstract}
%
\vspace{1.5ex}
