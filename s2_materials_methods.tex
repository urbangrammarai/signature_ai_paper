...
combination two components: the set of inputs used to make the prediction, and
...
probabilities generated by the CNN for each neighbor $j$ of chip $i$. This is
...
either are geographically contiguous or are nearest neighbors; while otherwise
they will be consider non-neighbors and receive a weight $w_{ij}=0$. To obtain
...
scores. The F1 is the harmonic mean between precission (proportion of
...
chips in the same class that are spatial neighbors (``joins'') over the total number of
pairs that are spatial neighbors.
Our neighborhood definition relies on two alternative spatial weights matrices:
one based on a distance threshold of $1Km$ ($W_{thr}$), and one that combines
the nearest neighbor with those defined by contiguity ($W_{union}$).
